-- Fix 2: Cast price (numeric) + review_count (integer) 
-- Les colonnes existantes sont numeric/integer mais Webflow envoie du text
...
DECLARE
  ...
  v_price_num NUMERIC;
BEGIN
  -- Convertir review_count en integer
  BEGIN
  ...
  BEGIN
    v_price_num := CAST(REGEXP_REPLACE(p_price, '[^0-9.,]', '', 'g') AS NUMERIC);
  EXCEPTION WHEN OTHERS THEN
    v_price_num := NULL;
  END;
  ...
  INSERT INTO partners (
    ...
    p_rating, v_review_count_int, v_price_num, p_promo_code, p_promo_percent,
